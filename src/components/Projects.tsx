...
export default function Projects({ projArray }: ProjectsProps) {
  const displayProjects = projArray.slice(0, 5); 
  ...
  {displayProjects.map((proj, index) => (
    <div
      key={index}
      className="cursor-pointer w-72 sm:w-64 rounded-lg overflow-hidden flex flex-col border border-white/30 bg-white/3 p-3 shadow-xl"
    >
      ...
      <h2 className="text-lg font-semibold mt-3">{proj.projectTitle}</h2>
